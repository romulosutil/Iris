...
import { comporRefs, mesclarPropsSlot } from "@/components/ui/primitives/slot";

type EpistemicState = "fact" | "suggestion" | "conquistado" | "candidato";

export interface InteractiveCardProps extends React.HTMLAttributes<HTMLElement> {
...
>(function InteractiveCard(
  ...
  const accentBar = destacado ? (
    <span
      aria-hidden
      className="absolute inset-x-0 top-0 h-2 rounded-t-md bg-[var(--action-primary)]"
    />
  ) : null;

  const titleHeading = titulo ? (
    <div className="font-display text-lg font-semibold text-[var(--text-primary)]">
      {titulo}
    </div>
  ) : null;
    ...
    mescladas.children = (
      <>
        ...
        {child.props.children ? (
          <span className="text-text-body block w-full">
  ...
  return (
    ...
    >
      ...
      {titleHeading}
      {children ? (
        <span className="text-text-body block w-full">{children}</span>
      ) : null}
    </Component>
  );
});
