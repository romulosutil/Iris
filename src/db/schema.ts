...
export const subscription = pgTable(
  ...
  (t) => [
    ...
    // sem token: em `free_tier` (nunca ativou), em `setup_pending` (fatura
    // hospedada emitida, cartão ainda não digitado) e em `canceled` (desistiu
    // ainda em `setup_pending` — sem essa cláusula o UPDATE de status estoura
    // 23514).
    check(
      "subscription_cartao_ativo_tem_token",
      sql`${t.metodoPagamento} <> 'cartao' OR ${t.status} IN ('free_tier', 'setup_pending', 'canceled') OR ${t.creditCardToken} IS NOT NULL`,
    ),
  ],
);
