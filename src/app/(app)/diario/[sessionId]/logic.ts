  ...
  try {
    ...
    try {
      const saida = await provider.extrair({
        ...
        // Modo explícito: o provider não pode depender de farejar `modo` dentro
        // do contexto canônico, que é `unknown` e pode nem ter sido montado.
        modo: prep.contexto?.modo,
      });
      drafts = saida.drafts;
