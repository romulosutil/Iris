...
describe.skipIf(!hasDb)(
  "actions.int.test.ts - supervisao actions integration tests",
  () => {
    beforeAll(async () => {
      if (!hasDb) return;
      ({ reconhecerAlerta, resolverAlerta, descartarAlerta } =
        await import("./logic"));
    ...
    test("1. reconhecerAlerta (coordenador) -> linha reconhecido + audit_log('reconhecimento_alerta', 'alerta')", async () => {
      ...
      const alertas =
        await owner`SELECT id, status, chave_natural FROM alerta WHERE patient_id = ${PAC}`;
      expect(alertas).toHaveLength(1);
      expect(alertas[0]!.status).toBe("reconhecido");
      expect(alertas[0]!.chave_natural).toBe(input.chaveNatural);

      const logs =
        await owner`SELECT acao, entidade, entidade_id FROM audit_log WHERE patient_id = ${PAC}`;
    ...
    test("2. reconhecerAlerta com terapeuta -> error, nenhuma linha gravada", async () => {
      ...
      const alertas =
        await owner`SELECT status FROM alerta WHERE patient_id = ${PAC}`;
    ...
    test("3. reconhecerAlerta 2x mesma chave -> 2a retorna CONCURRENCY_ERROR, sem 2a linha", async () => {
      ...
      const alertas =
        await owner`SELECT id FROM alerta WHERE patient_id = ${PAC}`;
      expect(alertas).toHaveLength(1);
    });
    ...
    test("4. resolverAlerta sem linha prévia -> INSERT resolvido + nota + audit", async () => {
      ...
      const alertas =
        await owner`SELECT status, nota, criado_por FROM alerta WHERE patient_id = ${PAC}`;
      expect(alertas).toHaveLength(1);
      ...
      const logs =
        await owner`SELECT acao, entidade FROM audit_log WHERE patient_id = ${PAC}`;
      expect(logs).toHaveLength(1);
    ...
    test("5. resolverAlerta sobre reconhecido -> UPDATE resolvido + atualizado_por = coordenador", async () => {
      ...
      // Depois resolve
      const r = await resolverAlerta(ctxCoord, {
        ...baseInput,
        nota: "Resolvido no papo",
      });
      expect(r.ok).toBe(true);

      const alertas =
        await owner`SELECT status, nota, atualizado_por FROM alerta WHERE patient_id = ${PAC}`;
      expect(alertas).toHaveLength(1);
      expect(alertas[0]!.status).toBe("resolvido");
    ...
    test("6. resolverAlerta sobre linha terminal -> CONCURRENCY_ERROR", async () => {
      ...
      // Resolve direto
      await resolverAlerta(ctxCoord, {
        ...baseInput,
        nota: "Primeira resolução",
      });

      // Tenta resolver de novo
      const r = await resolverAlerta(ctxCoord, {
        ...baseInput,
        nota: "Segunda tentativa",
      });
    ...
    test("7. descartarAlerta sem linha -> INSERT descartado + motivo + audit descarte_alerta", async () => {
      ...
      const alertas =
        await owner`SELECT status, motivo FROM alerta WHERE patient_id = ${PAC}`;
      expect(alertas).toHaveLength(1);
      expect(alertas[0]!.status).toBe("descartado");
      expect(alertas[0]!.motivo).toBe("Alerta irrelevante");

      const logs =
        await owner`SELECT acao FROM audit_log WHERE patient_id = ${PAC}`;
      expect(logs).toHaveLength(1);
