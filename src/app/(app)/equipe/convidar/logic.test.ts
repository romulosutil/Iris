...
describe("convidarUsuario — lógica de envio de e-mail de convite (#155)", () => {
  it("exige papel de coordenador", async () => {
    await expect(
      convidarUsuario(
        ctxTerapeuta,
        form({ nome: "Ana", email: "ana@iris.test", papel: "terapeuta" }),
      ),
    ).rejects.toThrow(/Acesso negado/);
  });

  it("valida campos obrigatórios do formulário", async () => {
    const resSemNome = await convidarUsuario(
      ctxCoord,
      form({ email: "ana@iris.test", papel: "terapeuta" }),
    );
    ...
    expect(resPapelInvalido.error).toBe(
      "Só é possível convidar terapeuta ou recepção por aqui.",
    );
  });

  it("chama provisionUser e dispara e-mail transacional com a senha temporária para NOVO usuário (#155)", async () => {
    vi.spyOn(provisioning, "provisionUser").mockResolvedValue({
      userId: "u-123",
      isNewUser: true,
    });
    const spyEmail = vi
      .spyOn(transacional, "enviarEmailTransacional")
      .mockResolvedValue({ enviado: true });

    const res = await convidarUsuario(
      ctxCoord,
      form({
        nome: "Dra. Paula",
        email: "paula@iris.test",
        papel: "terapeuta",
      }),
    );
  ...
  it("NÃO atualiza dados profissionais de usuário JÁ EXISTENTE ao enviar convite", async () => {
    ...
    vi.spyOn(transacional, "enviarEmailTransacional").mockResolvedValue({
      enviado: true,
    });
