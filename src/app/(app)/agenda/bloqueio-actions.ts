...
export async function criarBloqueioAction(_prev: BloqueioState, formData: FormData): Promise<BloqueioState> {
  ...
  try {
    ...
    await withTenant(ctx, (tx) => tx.insert(bloqueio).values({ clinicId: ctx.clinicId, ...validado.valor }));
    const caminho = formData.get("caminho")?.toString();
    if (caminho) revalidatePath(caminho);
    return { ok: true };
  } catch (err) {
    if (err instanceof RoleError) return { error: "Você não tem permissão para registrar bloqueios." };
    console.error("criarBloqueioAction:", err);
    return { error: "Não foi possível registrar o bloqueio. Tente novamente." };
  }
}

export async function removerBloqueioAction(_prev: BloqueioState, formData: FormData): Promise<BloqueioState> {
  const ctx = await getTenantContext();
  const id = formData.get("id")?.toString() ?? "";
  try {
    requireRole(ctx, "coordenador");
    await withTenant(ctx, (tx) => tx.delete(bloqueio).where(and(eq(bloqueio.clinicId, ctx.clinicId), eq(bloqueio.id, id))));
    const caminho = formData.get("caminho")?.toString();
    if (caminho) revalidatePath(caminho);
    return { ok: true };
  } catch (err) {
    if (err instanceof RoleError) return { error: "Você não tem permissão para remover bloqueios." };
