...
describe("Webhook Resend - processarWebhookResend", () => {
  ...
  describe("processarWebhookResend", () => {
    ...
    it("devolve 200 e emite log estruturado para email.complained sem vazar PII", async () => {
      ...
      const rawLogged = JSON.stringify(warnSpy.mock.calls);
      expect(rawLogged).not.toContain("usuario@exemplo.com");
      expect(rawLogged).not.toContain("Convite de equipe");
    });
    ...
    it("devolve 200 e loga info para outros eventos (ex: email.delivered, email.sent)", async () => {
      ...
      expect(infoSpy).toHaveBeenCalledWith(
        "[resend-webhook] evento recebido",
        expect.objectContaining({
          type: "email.delivered",
      ...
      const rawLogged = JSON.stringify(infoSpy.mock.calls);
      expect(rawLogged).not.toContain("outro@exemplo.com");
    });
  });
});
