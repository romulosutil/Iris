...
describe("notificacao.ts — canaisIndisponiveis (#126)", () => {
  ...
  test("estágio 2 sem EMAIL_PROVIDER_API_KEY registra e-mail indisponível", () => {
    delete process.env.EMAIL_PROVIDER_API_KEY;
    expect(canaisIndisponiveis(2)).toContain("email_responsavel_tecnico_indisponivel");
  });
  ...
  test("estágio 2 com EMAIL_PROVIDER_API_KEY configurada não registra indisponibilidade", () => {
    ...
    expect(canaisIndisponiveis(2)).not.toContain("email_responsavel_tecnico_indisponivel");
  });

  test("estágios 0 e 1 nunca checam e-mail (só existe no estágio 2)", () => {
    delete process.env.EMAIL_PROVIDER_API_KEY;
    expect(canaisIndisponiveis(0)).not.toContain("email_responsavel_tecnico_indisponivel");
    expect(canaisIndisponiveis(1)).not.toContain("email_responsavel_tecnico_indisponivel");
  });
});
