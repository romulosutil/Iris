  ...
  it("depois do vencimento fica negativo (não exibe banner)", () => {
    ...
    expect(
      diasRestantesDeTrial(
        ...
        7,
        TZ,
        new Date("2026-08-09T01:00:00-03:00"),
      ),
    ).toBe(-1);
...
describe("calcularStatusTrial", () => {
  ...
  it("sem 1º paciente e dentro do teto: aguardando, sem consumir dias", () => {
    const s = calcularStatusTrial(
      criadoEm,
      null,
      7,
      TZ,
      new Date("2026-08-10T09:00:00-03:00"),
    );
  ...
  it("no último dia do teto ainda está aguardando (fronteira de data civil)", () => {
    ...
    const s = calcularStatusTrial(
      ...
      7,
      TZ,
      new Date("2026-08-15T23:00:00-03:00"),
    );
  ...
  it("expirado é derivado ANTES do clamp — 0 dias restantes ainda é trial ativo", () => {
    ...
    const ultimoDia = calcularStatusTrial(
      criadoEm,
      trialComecoEm,
      7,
      TZ,
    ...
    const diaSeguinte = calcularStatusTrial(
      criadoEm,
      trialComecoEm,
      7,
      TZ,
