...
import {
  horasAgendadasPorDisciplina, horasRealizadasPorDisciplina,
  defasagem, temDefasagemSustentada, alocadoTerapeuta, vagoTerapeuta,
} from "./horas";
...
describe("horasRealizadasPorDisciplina", () => {
  it("média por semana ativa desde a 1ª sessão da disciplina", () => {
    ...
    const r = horasRealizadasPorDisciplina([
      { disciplina: "aba", duracaoMin: 60, agendadaPara: new Date("2026-07-06T12:00:00-03:00") },
      { disciplina: "aba", duracaoMin: 60, agendadaPara: new Date("2026-07-13T12:00:00-03:00") },
    ], agora);
...
describe("defasagem / sustentada", () => {
  it("defasagem = alvo - agendado", () => { expect(defasagem(12, 8)).toBe(4); });
  it("alerta quando agendado<alvo por ≥ limiar semanas", () => {
    expect(temDefasagemSustentada([{alvo:12,agendado:8},{alvo:12,agendado:10}], 2)).toBe(true);
    expect(temDefasagemSustentada([{alvo:12,agendado:8},{alvo:12,agendado:12}], 2)).toBe(false);
  });
});
describe("terapeuta", () => {
  it("alocado = Σ duracaoMin ÷ 60; vago = capacidade - alocado - bloqueado", () => {
    expect(alocadoTerapeuta([{disciplina:"aba",duracaoMin:60},{disciplina:"fono",duracaoMin:60}])).toBeCloseTo(2);
