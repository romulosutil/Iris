...
function buildDossie(overrides: Partial<PayloadConvenioBruto> = {}): PayloadConvenioBruto {
  return {
...
function buildInput(dossie: PayloadConvenioBruto): ConvenioNarrativoInput {
  return {
    paciente: { nome: "Paciente Teste" },
    periodo: { inicio: "2026-01-01", fim: "2026-01-31" },
    cabecalho: { operadora: "Operadora X", cid: "F84.0", finalidade: "Continuidade" },
    dossie,
  };
}
...
describe("StubConvenioNarrativoProvider", () => {
  it("C2: só emite números presentes no dossiê", async () => {
    ...
    const dossie = buildDossie({
      ...
      presenca: { sessoesRealizadas: 8, faltasJustificadas: 1, faltasNaoJustificadas: 0 },
      evidencias: [
        { data: "2026-01-05", metaOuDominio: "Comunicação", classificacao: "independente", autor: "Terapeuta A" },
        { data: "2026-01-12", metaOuDominio: "Comunicação", classificacao: "independente", autor: "Terapeuta A" },
        { data: "2026-01-19", metaOuDominio: "Comunicação", classificacao: "independente", autor: "Terapeuta A" },
      ],
    });
    const draft = await new StubConvenioNarrativoProvider().gerar(buildInput(dossie));
  ...
  it("evolucaoPorDominio: um item por domínio distinto, narrativa menciona a contagem", async () => {
    const dossie = buildDossie({
      evidencias: [
        ...
        { data: "2026-01-06", metaOuDominio: "Comunicação", classificacao: "dica_verbal", autor: "A" },
        { data: "2026-01-07", metaOuDominio: "Motor", classificacao: "independente", autor: "A" },
      ],
    });
    const draft = await new StubConvenioNarrativoProvider().gerar(buildInput(dossie));
    expect(draft.evolucaoPorDominio).toHaveLength(2);
    const comunicacao = draft.evolucaoPorDominio.find((e) => e.dominio === "Comunicação");
  ...
  it("C1 tom técnico: não usa termos infantilizados de comunicação com família", async () => {
    const dossie = buildDossie({
      evidencias: [{ data: "2026-01-05", metaOuDominio: "Comunicação", classificacao: "independente", autor: "A" }],
    });
    const draft = await new StubConvenioNarrativoProvider().gerar(buildInput(dossie));
