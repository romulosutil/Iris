...
| **D36** | **O contrato de ENTRADA do modo Terapia Convencional diverge do que o assembler emite.** Os 5 contextos do eval TC (`casos-de-teste-terapia-convencional.md`) trazem `paciente.resumo_repertorio` e `familia_abordagem`; `buildCanonicalContext` (`context-assembler.ts:59-97`) emite `paciente.idade_meses` + `metas_ativas: []` e nada disso. O `CONVENTIONAL_SYSTEM_PROMPT` promete ao modelo (seção `## Entradas`, item 3) entradas que ele não vai receber. O contrato de SAÍDA foi fechado no B3/PR #305; o de ENTRADA não. | O R9-TC depende de saber a família do terapeuta para decidir qual vocabulário NÃO importar. Sem o campo chegando, a regra vira sorte: o modelo espelha o vocabulário do diário por correlação, e o par TC-5a/TC-5b (o caso que sustenta o posicionamento de nicho) não tem como passar de forma confiável. Prometer uma entrada que não chega também degrada a obediência ao resto do prompt. | #98 · `context-assembler.ts:59` · `prompt.ts` (`## Entradas`) |
| **D37** | **`resumo_sessao` é descartado nos DOIS modos hoje; no TC isso foi consertado, no ABA não.** `diario/[sessionId]/logic.ts` grava só `drafts` — o `resumo_sessao` do contrato ABA nunca é persistido. No modo ABA é tolerável (as extrações são o artefato); fica registrado para decidir se o ABA deveria persistir o dele. | Sem decisão explícita, o campo continua sendo pedido ao modelo (custa tokens em toda extração) e jogado fora. Ou ele tem consumidor, ou sai do contrato — o estado atual é o pior dos dois. | #98/#305 · `logic.ts:440-455` · `agent-output-schema.ts:218` |
| **D38** | **O modo Terapia Convencional não entra na fila de validação por exceção do coordenador (V1-V5).** A exclusão é _implícita e correta_ — a fila lê `evidence`, e o modo nunca escreve lá — mas fica registrado que isso **confirma o Risco 4 do §4 do protocolo**: o modo perde o gatilho automatizado de auditoria clínica que o modo ABA tem. | Um resumo de terapia convencional aprovado pelo terapeuta não passa por nenhuma amostragem do coordenador. Para o nicho ABA a fila é a rede de segurança contra deriva do agente; no TC não há rede equivalente, e nada na UI sinaliza essa diferença ao coordenador. | #98 · `validacao/**` · protocolo TC §4 (Risco 4) |
| **D39** | **`when` da migração `0099` (`1786738757645`) segue a convenção "anterior + 1000" a partir da `0098` desta branch, e por isso é MENOR que o de migrações de outras branches abertas.** Medido em 15/08/2026: o Postgres local `:5433` (compartilhado entre worktrees) já tem `max(created_at) = 1786830635280`, e `pnpm db:migrate` **pulou a 0099 em silêncio**. Em `main`/prod o `max` aplicado é `1786736757645` (a `0097`), então a cadeia 0098→0099 aplica normalmente **hoje**. | Se qualquer outra branch com migração de `when` maior mergear em `main` ANTES do PR #305, a `0099` passa a ser ≤ o último aplicado e o Drizzle a **pula em silêncio** em produção — exatamente o bug #165 da `0055`. **Antes de mergear o #305: conferir `max(when)` do `_journal.json` de `main` e renumerar a `0099` se necessário.** | #98/#305 · `db/migrations/meta/_journal.json` · memória `drizzle-hand-migration-when-ordering` |
...
## 🧠 Sessão 15/08/2026 — Achado B3 do PR #305: contrato de saída do modo Terapia Convencional

**O que estava quebrado.** O modo falhava em **silêncio**. `extracoes: []` é saída válida no `agentOutputSchema`, e `resumo_sessao` não é persistido em lugar nenhum (`diario/[sessionId]/logic.ts` grava só `drafts`). Uma sessão de paciente convencional consolidava com **sucesso** e gravava **zero artefato** — tela de revisão vazia, nada indicando falha. Além disso, as 5 saídas esperadas do eval TC **falhavam no `parse`**, inclusive a do TC-2, que é violência.

**Decisões travadas com o Rômulo (não reabrir sem novo motivo):**

1. **`alerta_risco`: o doc cede ao código.** A forma implementada (#122 / R20) vence — `categoria` + `severidade` + `certeza` + `trecho_fonte` + `detalhe`, sem `presente`, ausência = campo omitido. Motivo: `severidade` é o campo que `app_criar_alerta_risco` usa para resolver o prazo do alerta no banco (`src/lib/risco/registrar.ts`); mudar o código quebraria o cálculo. §3 do protocolo e as 5 saídas do eval foram reescritos para a forma implementada.
2. **Severidade é ensinada no prompt do modo**, não deduzida. Sem a instrução explícita (`ideacao_passiva` / `ideacao_ativa_sem_plano` / `ideacao_ativa_com_plano`) a severidade sai por chute e o prazo do alerta clínico sai errado. Para `violencia_*` e `risco_a_terceiro` o mapeamento categoria→severidade é 1:1.
3. **O resumo entra como `sugerida`.** Mesma tela de revisão do modo ABA: o terapeuta aprova ou descarta. Mantém o humano no laço (princípio 3 do README) e atende o Risco 2 do §4 do protocolo.

**Estruturas novas.** `src/lib/extraction/conventional-output-schema.ts` (irmão do contrato ABA, que ficou **byte-idêntico** — é contra o JSON dele que o modelo de produção foi calibrado); subtipo `resumo_terapia_convencional` (migração `0099`, aditiva); `docs/agente/output-schema.json` passou a conter **os dois** contratos sob as chaves `protocol_driven` e `terapia_convencional` (sem campo `version`: versionar sem consumidor que leia a versão é cerimônia sem oráculo).

**Limite honesto de cobertura.** O **R9-TC** (vocabulário cruzado, critério de aprovação do par TC-5a/TC-5b) **não é testável com invoker fake** — depende do modelo vivo. Um "teste de R9-TC" com mock provaria apenas que o mock devolve o que o mock devolve. Pertence ao eval/bake-off, não ao `pnpm test`.

**Fora de escopo desta fatia (registrado, não feito):** `familia_abordagem` (campo + cadastro + migração — §2.2 marca como proposta não decidida); consentimento de adulto autoconsentindo (§5); constraint de paciente misto (AV-11); `historico_relevante` de temas (`context-loader.ts` manda `[]` fixo); gate automatizado de linguagem hedged (R4-TC); `DemoStubProvider` convencional; granularidade por tema em linhas separadas na revisão.

Débitos novos: **D36** (contrato de entrada divergente), **D37** (`resumo_sessao` descartado no modo ABA), **D38** (modo TC fora da fila V1-V5 — confirma o Risco 4 do §4), **D39** (ordenação do `when` da `0099` antes do merge).
