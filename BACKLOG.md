...
**Auditoria 360: só #558 segue aberta — e as 6 decisões já estão ratificadas.** Correção de medição própria desta sessão: uma primeira varredura (`grep -A 3` nas headers `## G-`) não alcançou as linhas "Ratificada pelo Rômulo em 03/09/2026" mais abaixo em cada seção e concluiu, errado, que nada estava decidido. Releitura integral de `.specs/features/558-cadeia-suporte-evidence/context.md` mostra o cabeçalho **"As 6 decisões estão FECHADAS"** e cada uma ratificada em 03/09/2026: G-1 (a) âncora única na cadeia · G-2 (a) índice do array, resolvida por medição · G-3 (a) etapa = linha em `evidence` · G-4 (b) hexágono + bloco próprio de rotinas · G-5 (a) linha de corte, sem backfill · G-6 (a) não classificado e reportado. Nada bloqueia mais a #558 do ponto de vista de decisão — falta gerar `tasks.md` e aplicar a label `jules` (`AGENTS.md` §5.2).
