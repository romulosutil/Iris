...
Verde medido em cada PR: `typecheck`/`lint` (0 erros)/`test`/`test:rls`. Migrações `0107` e `0108` verificadas via `pg_enum` (não só leitura do `.sql`). Nenhuma das 3 PRs foi mergeada ainda — empilhadas, aguardando revisão do Rômulo ou merge sequencial.
