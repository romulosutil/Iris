...
| **4** | `BACKLOG.md` (este arquivo)                                                                               | Débitos vivos (D1–D39) e log de sessões. Consulta, não leitura linear — venha buscar o histórico de uma decisão específica.                                                                                    |
...
| **D20** | ~~**`VERSAO_TERMO` estava declarado em dois lugares** — `src/lib/legal.ts` (que se diz "fonte única") e `src/app/(auth)/cadastro/logic.ts`. Corrigido na #191: o cadastro passou a importar de `@/lib/legal`.~~ **Fechado em 16/08/2026** — Trava estática em `src/lib/legal.test.ts` que varre recursivamente todo o código em `src/` (excluindo arquivos `.test.ts(x)`), garantindo que `VERSAO_TERMO` e `VERSAO_POLITICA` são declaradas exclusivamente em `src/lib/legal.ts`. O guard detecta `const|let|var` com ou sem anotações de tipo TypeScript (`const VERSAO_TERMO: string =`) e asserção positiva garantindo falha se a constante for deletada ou redeclarada em qualquer módulo novo ou existente. | A divergência era **invisível ao teste**: `legal.test.ts` comparava a constante de `legal.ts` com os markdown e nunca olhava a segunda cópia. Subir a versão num lado só faria o aceite do profissional gravar uma versão que **nenhum documento publicado tem** — em `professional_consent`, que é append-only. O resíduo foi sanado por varredura estática no CI com regex estrita contra mutação/redeclaração tipada em qualquer arquivo sob `src/`. | #191 · `src/lib/legal.ts` · `src/lib/legal.test.ts` |
