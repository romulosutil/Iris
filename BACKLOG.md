...
| **6b**  | Iris Audio Companion (Ditado de Voz ASR pós-sessão)      |                                                                                                                        📅 Fast-follow · escopo restrito a ditado pós-sessão (30/08) · provedor ASR ainda não decidido                                                                                                                         | Issue #72                |
...
## 🏁 Sessão 30/08/2026 — #72 (Iris Audio Companion): escopo restrito a ditado pós-sessão, "Modo Ambiente" adiado

Decisão do Rômulo: a issue #72, que hoje mistura dois modos ("Modo Ambiente" — gravar a sessão inteira — e "Ditado de Voz" — alternativa ao digitar), fica **só com ditado pós-sessão**. Fluxo: terapeuta grava áudio depois do atendimento (voz só do terapeuta, não do paciente), ASR transcreve, texto entra como rascunho pra revisão — é o fluxo que a spec da issue já desenha (Pill grava → server action transcreve → revisão humana obrigatória), sem streaming.

**"Modo Ambiente" (gravar a sessão inteira ao vivo) fica só registrado aqui, sem issue própria ainda.** Achados que travam essa ideia se for retomada:

1. **Captura voz do paciente, não só do terapeuta.** Consentimento atual (`termo-consentimento-titular-adulto.md` §8.1, `termo-consentimento-menor.md` §9.1 — ver D59) cobre ASR como recurso do terapeuta ditando; não cobre gravar a fala do paciente durante a sessão. Precisaria cláusula própria antes de qualquer código.
2. **Transcrição "ao vivo" (aparecendo durante a fala) exige streaming** (VAD + chunking incremental) — Whisper batch não serve. Sem GPU na VPS, Whisper `medium`/`large` roda mais devagar que tempo real em CPU pura (sessão de 45min podendo levar mais que 45min pra transcrever): infeasível como "ao vivo" com o hardware atual.

**Provedor de ASR: nenhuma decisão registrada ainda** — nem no BACKLOG nem na issue. `.env.example` e o corpo de #72 não nomeiam provedor. Direção discutida nesta sessão, não decidida: self-hosted (Whisper/faster-whisper na própria VPS) evitaria o gate de DPA por completo — [[fase6-lgpd-constraints|memória fase6]] trava o gate em "ASR **externa** = transferência internacional (Art. 33 LGPD)"; áudio que nunca sai da infra Iris não é transferência internacional, então não precisa de DPA com terceiro. Fica como opção a validar quando #72 entrar em spec — não é decisão travada.
