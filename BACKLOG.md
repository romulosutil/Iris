...
| **4** | `BACKLOG.md` (este arquivo)                                                                               | Débitos vivos (D1–D36) e log de sessões. Consulta, não leitura linear — venha buscar o histórico de uma decisão específica.                                                                                    |
...
- **G7/G0 escalam na 3ª recusa operacional do mesmo ciclo**, passando ao desfecho de G2. Sem limite, um banco que erra sempre é assinatura gratuita vitalícia — o mesmo buraco do G1 por outra porta. Três porque é o orçamento do `3R_7D`. Contagem por **ciclo**, zera quando o ciclo é pago.
...
3. **Duas decisões são do arquiteto, não do Rômulo, e mudam quando uma clínica real perde acesso:** a divergência deliberada da DoD em **G5** e o limite de **3** de G7/G0. Marcadas como proposta pendente de validação.

**A divergência de G5, explícita.** A DoD escrita diz "recusa terminal de conta não consome carência esperando um pagamento que não vem". Foi implementada a **intenção**, não a letra: o que não pode acontecer é **retentar** (por isso "vale gastar retentativa" = não). Consumir a carência é o que dá à clínica um prazo com data para refazer a autorização com outra conta; não carimbar deixaria a assinatura `active` para sempre, e encurtar a carência só para este grupo exigiria mecanismo novo sem ganho. É o único ponto de divergência; reverter é trocar uma linha da tabela, não o desenho.
