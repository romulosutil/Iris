...
| **D5**  | ~~Sandbox Asaas nunca exercitado ponta a ponta.~~ **Fechado em 03/08/2026; webhook de produção confirmado em 11/08/2026** — evento real entregue e gravado (ver sessão abaixo). `ASAAS_WEBHOOK_TOKEN` provisionada e **verificada por medição** em produção. Webhook de produção cadastrado na conta Asaas de produção e token `ASAAS_WEBHOOK_TOKEN` confirmado ativo pelo Rômulo (11/08) — o valor (mesmo truncado) fica só no runbook privado, nunca aqui (auditoria 360, `S-09`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Teste com dublê não cobre o dialeto do destino real — precedente direto: 18/18 verdes contra MinIO com zero cópia chegando na produção Oracle. **Confirmado na prática:** o `id` real vem como `evt_<hash>&<n>` (com `&`) e as datas dentro de `authorization` são `dd/MM/yyyy`, não ISO — nenhum dublê do repo usava esse formato.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | #36                                                                                                                                                                                         |
...
| **D41** | ~~**Cobrança apagada no painel do Asaas tranca a reativação, e a alternativa não está medida.**~~ **Fechado em 23/08/2026 por medição real em produção.** Cobrança de teste vencida (R$234,00, cliente-teste `DesignerS`, `cus_<teste>`) excluída manualmente no painel Asaas; `GET /v3/payments?customer=cus_<teste>` (produção) devolveu `totalCount: 0` — nenhum registro, nem com `deleted: true`. Controle no mesmo request contra cliente com cobrança viva (`cus_<controle>`) confirmou a API respondendo normalmente (`totalCount: 1`, `deleted: false`), descartando falso-positivo por chave/filtro quebrado. `GET /payments?customer=...` não ressuscita cobrança apagada — o gate pode liberar o `provider_charge_id` do ciclo com segurança quando a cobrança some do painel. O irmão menor (`DUNNING_RECEIVED` "recuperada" × "em recuperação") permanece não medido, fora do escopo desta medição. Os ids reais dos dois `customer` de produção usados na medição estão no runbook privado (auditoria 360, `S-09`: identificador de cliente de gateway não se versiona).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | O bloqueio conservador ("fale com o suporte") era a escolha segura, mas travava a clínica sem necessidade: medição em produção prova que reemitir após exclusão não gera cobrança duplicada/zumbi. Próximo passo (fora desta medição): destravar o gate para tratar `deleted: true` como o `404` já tratado — libera o `provider_charge_id` para nova emissão.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | #310 · `src/lib/billing/debito.ts` · `src/lib/billing/provider/asaas.ts`                                                                                                                    |
...
| **D54** | ~~**O componente `Alert` embute `border-l-[4px]`, o acento lateral que o Design System bane — em todo uso no produto.**~~ **Fechado em 21/08/2026 (PR #416).** Removidas as classes `border-l-[4px]`, `bordaEsquerda` e `bordaOutras` de `src/components/ui/alert.tsx`, alinhando todos os alertas da plataforma ao Design System Espectro Brutal com borda uniforme sólida de 2px (`border-2 border-[var(--status-{severidade}-border)]`). Criada suíte completa de testes unitários em `src/components/ui/alert.test.tsx` com guardrails estritos contra regressão visual e validação de acessibilidade. | Alinha o componente Alert às diretrizes do Espectro Brutal sem acentos laterais assimétricos artificiais. Validado por 100% de sucesso na suíte de testes unitários e acessibilidade. | `src/components/ui/alert.tsx` · `src/components/ui/alert.test.tsx` · `docs/ux/design-system-espectro-brutal.md` |
...
| **D72** | ~~**Proposta pendente de validação com o Rômulo — T11 (UI multi-clipe) e T12 (renderização do resultado transcrito) da #72 nunca foram implementadas.**~~ **Decidido em 31/08/2026:** issue de continuação **#494** aberta com T11/T12 e o resto do que falta; #72 fechada para não deixar uma issue meio-viva. O débito continua aberto no produto — só mudou de lugar de rastreio. Contexto original preservado abaixo. **Original:** Achado durante a bateria de verificação por mutação de T16 e confirmado na leitura de T17 (31/08/2026): `src/app/(app)/diario/[sessionId]/audio-local.tsx` continua a versão **single-clipe pré-#72** — não tem teto de 2min/auto-stop, não tem lista de clipes revisável (duração + descartar/regravar por item), não chama `enviarLoteAsrAction`/`obterEstadoLoteAction`/`obterLoteMaisRecenteAction` (`actions.ts`), e não gateia em `asrHabilitado()` (`src/lib/flags.ts`). Todo o backend/infra do Modo 2 (T01-T10, T13-T16: fila `SECURITY DEFINER`, storage efêmero, worker, agendador, sweeper, flag, mutação) está implementado e mergeado nesta PR — mas sem T11/T12 o pipeline não é alcançável por nenhum caminho de UI real. | Sem essas duas tasks, "Modo 2 implementado" descreve só a metade que não aparece pra ninguém: nenhuma terapeuta consegue gravar múltiplos clipes, ver o teto de 2min, revisar/descartar antes de enviar, nem ver o parágrafo transcrito voltar pro editor. O gap é de escopo de produto, não um bug — decidir se T11/T12 entram numa issue de continuação (nova numeração) ou reabrem #72 é decisão do Rômulo, não do executor. | #72 · #494 · `.specs/features/72-ditado-voz-asr/tasks.md` (T11, T12) · `src/app/(app)/diario/[sessionId]/audio-local.tsx` · `src/app/(app)/diario/[sessionId]/actions.ts` · `src/lib/flags.ts` |
...
| **D74** | ~~**R19 da #72 afirmava que a transcrição entra na exportação integral do acervo — falso.**~~ **Decidido em 31/08/2026 (opção C).** `src/lib/export/acervo/coletor.ts:84` põe `audio_capture` em `TABELAS_NEGADAS`, com teste travando em `coletor.test.ts:33`: a transcrição nunca entrou no ZIP. A metade do expurgo era verdadeira (`0128:162` faz `DELETE FROM audio_capture`). O fato que decidiu: `grep` por limpeza de `transcricao_texto` em `src/`, `db/` e `scripts/` só acha fixture de teste — **nenhum caminho de produção limpava o campo**, então o "rascunho intermediário" era permanente. **Decisão: limpar `transcricao_texto` quando o terapeuta aceita o texto no rascunho da nota**, estendendo ao texto a mesma regra de efemeridade que R11 já aplica ao áudio. A `session_note` continua sendo o registro que sobrevive e que já é exportado; `audio_capture` fica em `TABELAS_NEGADAS` de propósito. Descartadas: (A) exportar o texto bruto — entrega à paciente a versão que a máquina ouviu errado ao lado da corrigida; (B) guardar e não exportar, assumindo o risco de portabilidade. R19 reescrito em `.specs/features/72-ditado-voz-asr/spec.md` com a decisão datada; rastreabilidade passa a incluir T12. Implementação depende da UI (T11/T12) e vai junto, na #494. | O cenário 9 do plano de verificação (`tasks.md:312`) cobre exatamente "transcrição entra no expurgo e na exportação" e **nunca foi escrito** — `grep -E "expurgo\|acervo\|R19\|0128\|purgar"` nos testes da #72 dá 0 ocorrências, apesar de o PR #490 afirmar os 9 cenários cobertos. É a mesma assinatura de `executor-implementa-issue-nao-spec`: o requisito foi declarado coberto sem que ninguém medisse o lado da exportação, e o lado medido (expurgo) validou por metade. O teste do cenário 9 é parte de fechar esta decisão, não um extra. | #72 · #494 (Parte 4) · `.specs/features/72-ditado-voz-asr/spec.md` (R19) · `.specs/features/72-ditado-voz-asr/tasks.md:312` · `src/lib/export/acervo/coletor.ts` · `src/lib/export/acervo/coletor.test.ts` · `db/migrations/0128_retencao_expurgo_wiring.sql` |
| **D75** | ~~**Nenhum teste exercitava o provider REAL de extração — id de modelo aposentado passou verde por dias.**~~ **Fechado em 31/08/2026 (#510).** `pnpm test` roda com `EXTRACTION_LLM_ENABLED=false` (guardrail LGPD/D57, correto), então `resolveProvider` só devolvia `NullProvider`/`DemoStubProvider` — nenhum faz chamada de rede. E a suíte #395 (`casos-clinicos.llm.test.ts`) chama `createGeminiInvoker()` SEM argumento, herdando o modelo BARATO (`gemini-2.0-flash`): nunca tocou o id de produção. Resultado medido: quando o Google aposentou `gemini-2.5-flash` em 31/08/2026, 2260 testes unitários + 1281 de RLS ficaram verdes durante todo o tempo em que a extração de produção respondia 404 (PR #508). **Fechado com:** `src/lib/extraction/modelo-de-producao.llm.test.ts` — smoke do caminho de produção (`modeloDeExtracao()` + `LlmExtractionProvider` + `createGeminiInvoker`) contra a API real, com nota SINTÉTICA (sem PHI); `pnpm test:smoke`; workflow agendado diário `.github/workflows/smoke-provider-extracao.yml` que abre issue com label `bug` quando quebra (dedup: comenta na issue já aberta em vez de empilhar uma por dia). Oráculo é "não lança" — id aposentado, chave sem cota, schema recusado e resposta fora do contrato chegam todos como exceção. **Verificado por mutação contra o incidente real:** com `GOOGLE_EXTRACTION_MODEL=gemini-2.5-flash` o smoke falha com `404 ... is no longer available`; com o padrão, passa. Um segundo teste (custo zero) trava a deriva que cegou a #395: falha se o modelo exercitado convergir para o default barato do invoker. | O bug não é causado por commit nosso — é o Google aposentando um id — então nenhum gate de PR muda de estado quando acontece. Só relógio pega. Roda agendado e fora do `pnpm test`/CI de PR porque custa chamada real e exige `GOOGLE_API_KEY`; escopo é só o arquivo do smoke, nunca a suíte clínica cara da #395. | #510 · #508 · `src/lib/extraction/modelo-de-producao.llm.test.ts` · `.github/workflows/smoke-provider-extracao.yml` · `src/lib/extraction/provider.ts` (`modeloDeExtracao`) · memória `gate-de-flag-cega-a-suite-inteira` |
