...
## 🏁 Sessão 05/09/2026 — Fila Transacional Nativa no PostgreSQL (pg-boss) e Resolução de D73

Migração dos loops frágeis de agendamento em shell script (`agendador.sh`) e disparos HTTP expostos para um subsistema unificado e robusto de fila transacional nativa no PostgreSQL utilizando `pg-boss` (v12).

### O que foi entregue e verificado:

1. **Concorrência Estrita por Fila:**
   - Worker ASR configurado com `concurrency: 1` para proteção do modelo Whisper e memória da GPU/VPS.
   - Worker LLM de extração configurado com `concurrency: 5` para respeito aos limites e cotas da API do Gemini.
2. **Resiliência & Retries Controlados:**
   - Políticas de retry com backoff exponencial e limite estrito de 3 tentativas, eliminando loops infinitos.
   - Roteamento automático para Dead-Letter Queue (`dlq`) após exaustão das tentativas.
3. **Dead-Letter Queue (DLQ) Sanitizada:**
   - Handler da DLQ emite log estruturado (`pino`) com sanitização ativa (`logarErroSemPII`), garantindo ausência de PII de pacientes/sessões.
4. **Idempotência & Atomicidade:**
   - Enfileiramento transacional atômico usando `fromDrizzle(tx, sql)` dentro das transações Drizzle de persistência de áudio (`diario-asr.ts`) e consolidação de diário (`diario-consolidacao.ts`), eliminando o risco de dual-write.
   - Idempotência garantida via `singletonKey` (por `loteId` para ASR e `sessionId` para LLM).
5. **Cancelamento Graceful & Heartbeat:**
   - Propagação de `AbortSignal` com verificação periódica nos handlers de ASR e LLM e renovação de heartbeat configurada.
6. **Ciclo de Vida & Runners:**
   - Runner standalone `scripts/queue-worker.ts` com gerenciamento de sinais `SIGINT`/`SIGTERM` para execução autônoma em VPS/container.
   - Hook condicional de inicialização no `src/instrumentation.ts` controlado pela variável `QUEUE_WORKER_ENABLED`.
   - `infra/asr/agendador.sh` descontinuado e substituído pelo comando `pnpm queue:work`.
7. **DDL e Isolamento de Segurança:**
   - Migração `0154_pgboss_schema.sql` com o schema `pgboss` e concessão restrita de privilégios para `app_role`, mantendo `migrate: false` em runtime pelo cliente para garantir o princípio de privilégio mínimo sob RLS.

### Verificações realizadas:

- `pnpm format`: arquivos formatados com sucesso via Prettier.
- `pnpm typecheck`: 0 erros de tipagem TypeScript.
- `pnpm test`: 372 arquivos / 3.188 testes unitários passando 100%.
- `pnpm test:rls`: 162 arquivos / 1.424 testes de integração e RLS passando 100%.
