...
| **D11** | ~~**Estratégia de Ativo de Dados & Indexação RAG (#120)** — pipeline de tokenização e treinamento de IA sobre históricos exportados.~~ **Especificado e Implementado em 16/08/2026** (`src/lib/rag/`). Spec em `docs/superpowers/specs/2026-08-16-d11-issue-120-pipeline-rag-tokenizacao-treinamento-design.md`. | Pipeline completo de de-identificação LGPD (`sanitizer.ts`), tokenização/orçamento de contexto (`tokenizer.ts`), chunking hierárquico com metadados (`chunker.ts`), store vetorial multi-tenant (`vector-store.ts`), busca híbrida com BM25 + RRF + MMR (`retriever.ts`), projeção de `historico_relevante` para o Agente de Extração (Camada 1 / R14), ingestor de prontuários exportados (`dossier-loader.ts`) e particionamento anti-leakage por paciente para datasets SFT/DPO (`training-pipeline.ts`). 9 suítes e 30 testes unitários/E2E 100% verdes. | #120 · `src/lib/rag/`                                                                                                                                                                       |
