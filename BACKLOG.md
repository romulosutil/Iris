...
| **D3b** | **`salvarConfigEmergencia` é no-op silencioso.** `src/app/(app)/clinica/emergencia/logic.ts:129` faz `UPDATE clinic` pelo pool da aplicação (`app_role`), mas a 0002 só deu a `app_role` uma policy `FOR SELECT` em `clinic`, com `FORCE ROW LEVEL SECURITY`. Descoberto ao instruir a #188.                                                                                                                                  | Medido: `UPDATE 0`. A tela salva, grava `audit_log`, devolve `{ ok: true }` — e nada muda no banco. Responsável técnico e protocolo de emergência da clínica **nunca foram gravados em produção**. Não há `.int.test.ts` cobrindo o caminho.                                                                                        | #212 · fix é `SECURITY DEFINER` espelhando o predicado da leitura, não policy nova (`AGENTS.md`)              |
...
| **D15** | **Critério (c) de "paciente ativo" não é congelado no fim do ciclo.** `billing_apurar_ciclo` (`0071:320`) lê `p.arquivado_em IS NULL` **no instante da apuração**, enquanto (a) `criado_no_ciclo` e (b) `interacao_no_ciclo` já são congelados em `[inicio, fim)`. Como `fecharCiclosVencendo` seleciona por `ciclo_atual_fim <= agora` com tick de 1h, existe uma fresta entre o fim do ciclo e a apuração.                                                                                                | Achado na revisão da PR #214. A varredura diária de auto-arquivamento caindo nessa fresta tira do ciclo um paciente que ficou **não-arquivado o ciclo inteiro**: a clínica perde a cobrança do ciclo todo dele e `billing_cycle_patient` grava que ele não estava ativo — falso, e esse memorial é a peça de defesa de fatura contestada. Apurar o mesmo ciclo às 03h ou às 04h passa a dar valores diferentes. Erro é de **subcobrança**, por isso não bloqueou o merge. | #216 · fix é `arquivado_em IS NULL OR arquivado_em >= v_fim` nos dois pontos da função |
...
- **Issue #188 — Revogação de Privilégios `GRANT` em `clinic` (Débito D3):** ✅ **Concluído em 06/08/2026** — [Issue #188](https://github.com/romulosutil/Iris/issues/188), spec em [`docs/superpowers/specs/2026-08-03-issue-188-revogacao-grant-clinic-design.md`](docs/superpowers/specs/2026-08-03-issue-188-revogacao-grant-clinic-design.md). Migração `0079_clinic_grant_coluna_a_coluna.sql`: `REVOKE INSERT, UPDATE, DELETE ON clinic FROM app_role` + `GRANT UPDATE` nas 12 colunas de configuração da clínica. `DELETE` entrou junto porque o débito D3 o nomeia e nenhum caminho da aplicação apaga clínica — a spec só citava `INSERT`/`UPDATE`. As 7 imutáveis (`id`, `responsavel_conta_id`, `is_demo`, `trial_comeco_em`, `trial_dias`, `isento_trial`, `criado_em`) passam a ser negadas **por privilégio**, antes da RLS ser avaliada. `iris_auth` (signup) e a role dona não foram tocadas. Verificado por medição: `information_schema` mostra `SELECT` como único privilégio de tabela de `app_role`, `has_column_privilege(... 'isento_trial','UPDATE')` = `false`, e as tentativas reais devolvem `permission denied for table clinic` em vez de zero linhas em silêncio. Regressão travada em `src/db/rls-hardening-px.int.test.ts` (30 testes verdes). **D3 sai da tabela de débitos abertos.**
  - 🔴 **Achado colateral — [#212](https://github.com/romulosutil/Iris/issues/212) (novo débito D3b):** ao instrumentar a #188, mediu-se que `salvarConfigEmergencia` nunca gravou nada. Ver linha D3b na tabela de débitos.
