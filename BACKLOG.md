...
**Auditoria 360: só #558 segue aberta.** Travada em 6 decisões de produto (G-1 a G-6, `context.md` da spec #574). Verificado — **nenhuma decisão está registrada** em lugar nenhum do repo: 0 comentários na issue #558, PR #574 só tem o review automático do bot Jules (nenhum comentário do Rômulo), `context.md` sem commit desde o merge (`972e687d`), grep por `G-1`..`G-6` em `BACKLOG.md`/`docs/produto/*.md` = 0 hits. Se as decisões foram tomadas em conversa fora do repo, precisam ser transcritas aqui ou na issue antes da label `jules` (`AGENTS.md` §5.2) — senão o executor escolhe por elas (precedente #285).
