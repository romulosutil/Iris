...
| **6b**  | Iris Audio Companion (Ditado de Voz ASR pós-sessão)      |                                                                                                                     📅 Fast-follow · V1 restrito ao Modo 2, provedor self-hosted (sem custo) — spec `2026-08-02-issue-72-ditado-voz-asr-design.md`                                                                                                                      | Issue #72                |
...
**Decisão real desta sessão, com a restrição nova do Rômulo (V1 sem custo):**

- **Google sai do V1 por custo, não por decisão técnica revertida.** `dpa-asr-audio.md` §2 exige tier **pago** pra ativar Zero Data Retention (free tier treina modelo com o conteúdo — incompatível com dado de saúde). Isso nunca foi "sem custo"; a restrição nova descarta a opção já aprovada.
- **V1 usa self-hosted (Whisper/faster-whisper) na VPS já paga.** Efeito colateral bom, não só custo zero: sem provedor externo, não há transferência internacional (Art. 33 LGPD) — o capítulo inteiro de DPA/SCC/ZDR fica sem objeto pro V1, não só mais barato.
- **Escopo do V1 = só Modo 2 (ditado assíncrono pós-sessão).** Modo 3 (gravação de sessão inteira) sai do V1 por decisão de produto (reduzir superfície de entrega), não porque falte design ou consentimento — os dois já existem e continuam válidos se for retomado. Motivo técnico real pra não entrar agora: precisaria streaming (VAD + chunking) pra ser "ao vivo" de verdade, e Whisper batch em CPU sem GPU roda mais devagar que tempo real pra sessão de 45min+.

Nota de atualização (não reescrita) adicionada nos dois documentos técnicos, mesmo padrão do D67 — texto original intocado, divergência registrada à parte. Google volta a ser opção se self-hosted não performar em PT-BR clínico, ou se Modo 3 for retomado com volume que a VPS não aguente.
