...
| **D64** | **`infra/arquivamento/` está fora do teste de carga das imagens de infra.** `scripts/ci/carga-imagens-infra.sh` cobre `escalonamento`, `backup`, `billing` e — desde #352 — `retencao`; **não** cobre `arquivamento`, apesar de essa imagem ter exatamente o mesmo desenho de risco (COPY arquivo a arquivo + `npm install postgres@3.4.9` à mão, sem enxergar o `node_modules` do repo). O `paths` do workflow também não lista `infra/arquivamento/**`. Descoberto em #352 ao acrescentar o alvo `retencao`; **não tapado de carona**, porque acrescentar um alvo é mudança no CI de outro serviço. | É o buraco exato que a #126 explorou: um `import` novo em `scripts/auto-arquivamento.mjs` passa por `pnpm test`/`typecheck`/`lint` verdes (rodam contra a árvore do repo) e derruba a imagem em produção com `ERR_MODULE_NOT_FOUND` — import de topo não cai em `try/catch`, não há degradação graciosa. E o sintoma no produto é mudo: arquivamento parado é indistinguível de "ninguém passou dos 90 dias", com a fatura seguindo cobrando paciente inativo. O fechamento é ~30 linhas, copiando `carga_retencao()`. | #157 · #126 · #352 · `scripts/ci/carga-imagens-infra.sh` · `.github/workflows/carga-imagens-infra.yml` · `infra/arquivamento/Dockerfile` |
