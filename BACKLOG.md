...
## 🏁 Sessão 24/08/2026 (3ª) — D45/D46 fechados, PR #462 aberto e CONFLICTING

- **Status:** D45 e D46 marcados fechados no texto do débito (linhas 90-91), mas o commit está só na branch `fix/446-d46-loga-envelope-retentativa-inesperado`, PR #462 aberto contra `main`, `mergeable: CONFLICTING`. **Fechamento no BACKLOG.md é sobre o código escrito, não sobre o merge** — não tratar D45/D46 como resolvidos em produção até o PR resolver o conflito e mergear.
- Arquivo `docs/daily-summary/2026-08-24.md` untracked no working tree — não commitado, não faz parte deste PR.
- **Auditoria dos arquivos-base (AGENTS.md/CLAUDE.md) sobre atualização obrigatória de Grafo/Backlog/Checkpoint/GitHub:**
  - **Backlog + Checkpoint:** já imperativo em `AGENTS.md` §8 — fim de sessão com alteração relevante exige atualizar `BACKLOG.md` e, se contexto ~50 msgs, `checkpoint.md`. Nada a mudar.
  - **Grafo (graphify):** imperativo já existe, mas só no `CLAUDE.md` global do usuário (`~/.claude/CLAUDE.md`), não no `AGENTS.md`/`CLAUDE.md` do repo — regra "toda tarefa concluída exige `git commit` + `git push` + `graphify update .`". Como Jules e outros agentes seguem `AGENTS.md` como porta de entrada, essa regra pode não chegar a eles. **Gap identificado, não corrigido nesta sessão** — decisão de produto (se vale replicar no `AGENTS.md`) fica para o Rômulo.
  - **GitHub:** imperativo em `AGENTS.md` §5.3 (toda dívida/bug/melhoria vira Issue) e §5.4 (PR nasce Draft). Nada a mudar.
