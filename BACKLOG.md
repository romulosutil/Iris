...
| **D86** | **AÇÃO DO RÔMULO, não é código: o segredo `GOOGLE_API_KEY` nunca foi configurado no repositório, e por isso o smoke da #510 NUNCA rodou uma vez sequer.** Medido em 06/09/2026: `gh secret list` devolve só `JULES_API_KEY`, e os 6 runs agendados de 01→06/09/2026 (`33516534507`…`34033761936`) morreram todos no passo `Verifica que GOOGLE_API_KEY está configurada`, antes de qualquer chamada ao Gemini. **Correção:** Settings → Secrets and variables → Actions → New repository secret, nome `GOOGLE_API_KEY`, chave da Google AI Studio; depois **Actions → Smoke do provider de extração → Run workflow** e conferir que o run chega no passo `Smoke do provider de extração`. Nenhum deploy de código. | A proteção está **inerte desde que nasceu**. `MODELO_EXTRACAO_PADRAO = "gemini-3.6-flash"` (`src/lib/extraction/provider.ts:106`, trocado na PR #508 em 31/08/2026) nunca foi exercitado contra a API de verdade por nada neste repo — `pnpm test` exclui `*.llm.test.ts` de propósito, e o único relógio que perguntaria ao Google é este smoke. É exatamente a memória `gate-de-flag-cega-a-suite-inteira`: id de modelo aposentado passa verde até produção. Se o Google aposentar esse id, o primeiro sinal seria a extração caindo em `pendente_reprocessamento` sem erro visível para a terapeuta. O alerta ficou honesto nesta sessão, mas alerta honesto sobre proteção inerte continua sendo proteção inerte — **o débito só fecha com o segredo configurado e um run verde, não com o merge da PR.** | #524 · #510 · `.github/workflows/smoke-provider-extracao.yml` · `scripts/ci/alerta-smoke-extracao.mjs` · `src/lib/extraction/provider.ts:106` · `src/lib/extraction/modelo-de-producao.llm.test.ts` · memória `gate-de-flag-cega-a-suite-inteira` |
...
**Como ficou:** o texto saiu do `script:` inline para `scripts/ci/alerta-smoke-extracao.mjs` — função pura `montarAlerta({ passoSegredo, url, agora })` → `{ titulo, corpo, comentario }`. Motivo de existir como módulo: dentro do `script:` do YAML **nada roda no `pnpm test`**; fora, roda. O workflow importa por `pathToFileURL(GITHUB_WORKSPACE + …)` e continua dono das chamadas à API do GitHub.

**Mutação executada:** trocar o predicado `passoSegredo === "failure"` (ou seja, voltar ao corpo único) derruba **5 dos 14** casos de `scripts/ci/alerta-smoke-extracao.test.mjs` — exatamente os que dizem "não afirma nada sobre o provider", "nada foi medido", "nomeia a correção real", "#510 está inerte" e "o comentário também não afirma causa". Os 9 restantes sobrevivem, e é por isso que eles sozinhos não bastariam.

**Verificação além da suíte:** o corpo do `script:` do YAML foi extraído com um parser de YAML, compilado como `AsyncFunction` e **executado** com dublês de `github`/`context`/`core` e `GITHUB_WORKSPACE` apontando para o worktree. Provou três coisas que asserção de substring não prova: o `await import()` resolve o módulo pelo caminho que o workflow nomeia; os dois `passoSegredo` produzem os dois títulos; e, com a issue de segredo já aberta, uma falha REAL do Gemini **cria issue nova** (`criadas: 1 comentadas: 0`) em vez de comentar na thread errada. O harness não ficou no repo — exigiria `yaml` como dependência direta, escopo que a issue não pediu; o teste permanente prende a fiação por caminho + `existsSync`.
...
**Próximo passo:** Rômulo configurar `GOOGLE_API_KEY` (D86). Enquanto não configurar, o robô passa a abrir/comentar numa issue nova e honesta, cujo título é `Smoke do provider de extração não roda — GOOGLE_API_KEY ausente no repositório`.
