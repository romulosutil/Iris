...
| **7**   | Self-Service & Growth (onboarding + pagamento autônomo)  | 🚧 Em construção (trial #175 ✅ · arquivamento #174 parcial · billing Asaas: Fases A+B+C fechadas, PR #244 mergeada em `main` e deployada em produção (11/08) — `0090` (backfill) e `0091` (drop da tabela do MP) **verificadas por medição em produção**: `subscription.provider` sem default, `mercadopago_webhook_event` inexistente) | Issue #36                |
...
| **D23** | ~~**`app.user_role` e `app.user_id` continuam resolvidos com `current_setting()` cru** — 11 funções `SECURITY DEFINER` (`app_purgar_paciente`, `app_purgar_report`, `app_aplicar_snapshot`, `app_is_on_team`, `app_report_visivel`, …) e as ~39 policies que a `0085` reescreveu só no termo de clínica. GUC ausente → `42704`, exatamente o erro que o D16 fechou do lado do tenant.~~ **Fechado em 11/08/2026** — migração `0093_user_role_id_helpers.sql` criou 6 helpers de papel e identidade (`app_user_role_nao_resolvido`, `app_user_id_nao_resolvido`, `app_user_role_atual`, `app_user_id_atual`, `app_user_role_exigido`, `app_user_id_exigido`) com `P0001` diagnóstico e regex guard contra `22P02`. Reescrita de 6 funções DEFINER (`app_alerta_risco_visivel`, `app_session_clinica_visivel`, `app_salvar_config_emergencia`, `app_salvar_cpf_cnpj_clinica`, `app_desarquivar_paciente`, `app_criar_alerta_risco`). Guards de CI em `db/tests/clinic-id-helper-rls.int.test.ts` travam contra regressão e oráculo exato por função. | Deixado fora da `0087` **de propósito**, não por esquecimento: estes são gates de **papel**, e o fix não é mecânico como o do tenant. `current_setting('app.user_role', true)` troca "estoura" por "`NULL` → nega", que é fail-closed e portanto seguro; mas dentro de uma função `SECURITY DEFINER` — onde o guard interno **é** a fronteira de autorização — isso é mudança de semântica de autorização, e merece ser decidida por si, com trava por função, não de carona numa varredura de tenant. O `app.user_id` é o caso mais delicado: ele é castado para `uuid` em vários pontos, então `missing_ok` sozinho não elimina o `22P02` — precisaria de um `app_user_id_atual()` irmão do de clínica. Risco atual é baixo (`withTenant()` sempre seta os três GUCs juntos), mas a assimetria é uma armadilha para a próxima função escrita. | `0087` (comentário de escopo) · `0093_user_role_id_helpers.sql` · `db/tests/clinic-id-helper-rls.int.test.ts` |
