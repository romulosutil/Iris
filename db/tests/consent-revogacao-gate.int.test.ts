...
async function erroDe(p: Promise<unknown>): Promise<string> {
  try {
    ...
    while (atual && typeof atual === "object") {
      const o = atual as { message?: unknown; detail?: unknown; cause?: unknown };
  ...
  throw new Error("esperava rejeição do Postgres, mas a operação foi bem-sucedida");
}
...
async function travado(c: TenantContext, patientId: string) {
  const r = await withTenant(c, (tx) =>
    tx.execute(sql`SELECT app_prontuario_somente_leitura(${patientId}::uuid)::text AS v`),
  );
  return (r as unknown as { v: string }[])[0]?.v;
}

async function finalidadeRevogada(c: TenantContext, patientId: string, finalidade: string) {
  const r = await withTenant(c, (tx) =>
    tx.execute(sql`SELECT app_finalidade_revogada(${patientId}::uuid, ${finalidade})::text AS v`),
  );
  return (r as unknown as { v: string }[])[0]?.v;
}
...
describe.skipIf(!hasDb)("0052/0053 · revogação de consentimento e gate de prontuário", () => {
  beforeAll(async () => {
  ...
  test("caso 1b · terapeuta continua SEM INSERT em patient_alvo_disciplina (policy reescrita)", async () => {
    ...
    ).toMatch(/row-level security policy for table "patient_alvo_disciplina"/);
  ...
  test("caso 2 · cross-tenant continua bloqueado nas tabelas tocadas", async () => {
    ...
    ).toMatch(/row-level security policy for table "patient_clinical_profile"/);
  ...
  test("caso 3 · revogar → nova concessão de regime → escrita clínica volta", async () => {
    ...
    expect(await travado(coord, P_MENOR)).toBe("false");
    expect(await inserirGoal(P_MENOR, "depois do reconsentimento")).toHaveLength(1);
  ...
  test("caso 4 · menor revogado que completa 18 e autoconsente destrava", async () => {
    ...
    expect(await erroDe(inserirGoal(P_18, "travado por maioridade pendente"))).toMatch(
      /row-level security policy for table "goal"/,
    );
    ...
    expect(await inserirGoal(P_18, "destravado pelo autoconsentimento")).toHaveLength(1);
  });
  ...
  test("caso 16 · ex-menor adulto que revoga o autoconsentimento NÃO trava (§13)", async () => {
    ...
    expect(await travado(coord, P_18)).toBe("false");
    expect(await inserirGoal(P_18, "adulto revogado ainda grava")).toHaveLength(1);
  });
  ...
  test("caso 5 · adulto com finalidade revogada: session/session_note gravam; extração e exportação não", async () => {
    ...
    expect(await finalidadeRevogada(coord, P_ADULTO, "uso_ia_processamento")).toBe("false");
    expect(await inserirExtraction(SESS_ADULTO, "antes da revogação")).toHaveLength(1);
    ...
    expect(await finalidadeRevogada(coord, P_ADULTO, "uso_ia_processamento")).toBe("true");
    expect(await finalidadeRevogada(coord, P_ADULTO, "exportacao_relatorios")).toBe("true");
    ...
    expect(await erroDe(inserirExtraction(SESS_ADULTO, "depois da revogação"))).toMatch(
      /row-level security policy for table "extraction"/,
    );
  ...
  test("caso 17 · paciente sem NENHUMA linha de finalidade continua extraindo", async () => {
    const linhas = await owner!`SELECT 1 FROM consent WHERE patient_id = ${P_MENOR}
      AND tipo::text IN ('uso_ia_processamento','exportacao_relatorios')`;
    expect(linhas).toHaveLength(0);

    await desrevogar(P_MENOR);
    expect(await finalidadeRevogada(coord, P_MENOR, "uso_ia_processamento")).toBe("false");
    expect(await inserirExtraction(SESS_MENOR, "sem linha de finalidade")).toHaveLength(1);
  ...
  test("caso 18 · finalidade revogada e reconsentida com linha nova volta a permitir extração", async () => {
    ...
    expect(await finalidadeRevogada(coord, P_ADULTO, "uso_ia_processamento")).toBe("true");
    expect(await erroDe(inserirExtraction(SESS_ADULTO, "finalidade revogada"))).toMatch(
      /row-level security policy for table "extraction"/,
    );
    ...
    expect(await finalidadeRevogada(coord, P_ADULTO, "uso_ia_processamento")).toBe("false");
    expect(await inserirExtraction(SESS_ADULTO, "finalidade reconsentida")).toHaveLength(1);
  });
  ...
  test("caso 19 · com exportacao_relatorios revogada, report_select continua funcionando", async () => {
    ...
    expect(await finalidadeRevogada(coord, P_MENOR, "exportacao_relatorios")).toBe("true");
    ...
    const lidos = await withTenant(coord, (tx) =>
      tx.execute(sql`SELECT id FROM report WHERE patient_id = ${P_MENOR}::uuid`),
    );
  ...
  test("caso 6 · DELETE em extraction não apaga com prontuário travado, e apaga destravado", async () => {
    ...
    const apagadasTravado = await withTenant(ter, (tx) =>
      tx.execute(sql`DELETE FROM extraction WHERE id = ${alvo}::uuid RETURNING id`),
    );
    expect(apagadasTravado).toHaveLength(0);
    expect(await owner!`SELECT 1 FROM extraction WHERE id = ${alvo}`).toHaveLength(1);
    ...
    expect(await owner!`SELECT 1 FROM extraction WHERE id = ${alvo}`).toHaveLength(0);
  });
  ...
  test("caso 7 · com prontuário travado, SELECT continua em todas as tabelas do gate", async () => {
    ...
    for (const tabela of porPaciente) {
      ...
      expect(linhas.length, `SELECT em ${tabela} com prontuário travado`).toBeGreaterThan(0);
    }

    // Tabelas cujo vínculo é por session_id — lidas pelo terapeuta dono.
    for (const tabela of ["session_note", "extraction", "audio_capture", "session_protocol_scope"]) {
  ...
  test("caso 8 · app_aplicar_candidatura e app_criar_alerta_risco param com paciente travado", async () => {
    ...
    await revogar(P_MENOR, C_MENOR);
    expect(await erroDe(cand())).toMatch(/somente-leitura: consentimento revogado/);
  ...
  test("caso 8b · app_aplicar_snapshot também para com paciente travado", async () => {
    const snap = () =>
      withTenant(coord, (tx) =>
        tx.execute(sql`SELECT app_aplicar_snapshot(${P_MENOR}::uuid, 7, '{}'::jsonb, '{}'::jsonb)`),
      );
    await desrevogar(P_MENOR);
    await expect(snap()).resolves.toBeDefined();
    await revogar(P_MENOR, C_MENOR);
    expect(await erroDe(snap())).toMatch(/somente-leitura: consentimento revogado/);
    await desrevogar(P_MENOR);
  });
  ...
  test("caso 9 · app_purgar_paciente funciona em paciente com revogação registrada", async () => {
    ...
    expect(await owner!`SELECT 1 FROM consent WHERE patient_id = ${P_PURGA}`).toHaveLength(2);

    await withTenant(coord, (tx) =>
      tx.execute(sql`SELECT app_purgar_paciente(${P_PURGA}::uuid, 'teste de expurgo com revogação')`),
    );

    expect(await owner!`SELECT 1 FROM consent WHERE patient_id = ${P_PURGA}`).toHaveLength(0);
    expect(await owner!`SELECT 1 FROM patient WHERE id = ${P_PURGA}`).toHaveLength(0);
  });
  ...
  test("caso 10 · empate em assinado_em é desempatado por id DESC, de forma determinística", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      expect(await finalidadeRevogada(coord, P_TIE, "uso_ia_processamento")).toBe("true");
    }
    ...
    for (let i = 0; i < 3; i++) {
      expect(await finalidadeRevogada(coord, P_TIE, "uso_ia_processamento")).toBe("false");
    }
  });
  ...
  test("caso 11c · curatela SEM instrumento é rejeitada; COM instrumento passa", async () => {
    ...
    expect(await owner!`SELECT 1 FROM consent WHERE id = ${ok}`).toHaveLength(1);
    await owner!`DELETE FROM consent WHERE id = ${ok}`;
  });
  ...
  test("caso 14 · UPDATE e DELETE em consent continuam negados a app_role", async () => {
    expect(
      await erroDe(
        withTenant(coord, (tx) =>
          tx.execute(sql`UPDATE consent SET versao_termo = 'hackeada' WHERE id = ${C_CHK}::uuid`),
        ),
      ),
    ...
    await owner!`DELETE FROM consent WHERE patient_id = ${P_CHK}
      AND tipo::text = 'revogacao_consentimento'`;
    expect(await owner!`SELECT versao_termo FROM consent WHERE id = ${C_CHK}`).toEqual([
      { versao_termo: "menor-v1" },
    ]);
  });
});
