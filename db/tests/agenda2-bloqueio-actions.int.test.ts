...
describe.skipIf(!hasDb)("bloqueio actions — validação + persistência", () => {
  ...
  afterAll(async () => { await owner?.end(); await appSql?.end(); vi.restoreAllMocks(); });
  ...
  test("cria bloqueio de paciente válido", async () => {
    const r = await actions.criarBloqueioAction({}, fd({
  ...
  test("rejeita paciente sem patientId (I-B5) sem tocar o banco", async () => {
    const r = await actions.criarBloqueioAction({}, fd({
      escopo: "paciente", dataInicio: "2026-07-20", dataFim: "2026-08-10", motivo: "x",
    }));
  ...
  test("rejeita dataFim < dataInicio", async () => {
    const r = await actions.criarBloqueioAction({}, fd({
      escopo: "clinica", dataInicio: "2026-08-10", dataFim: "2026-07-20", motivo: "Feriado",
    }));
