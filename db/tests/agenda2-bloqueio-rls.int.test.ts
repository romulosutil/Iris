...
describe.skipIf(!hasDb)("Agenda 2.0 · RLS de bloqueio", () => {
  ...
  test("coordenador cria bloqueio de clínica (alvos null) e lê", async () => {
    const [row] = await withTenant(ctxCoordA, (tx) =>
      tx.insert(schema.bloqueio).values({
        clinicId: CLINIC_A, escopo: "clinica", dataInicio: "2026-12-24",
        dataFim: "2026-12-26", motivo: "recesso de fim de ano",
      }).returning({ id: schema.bloqueio.id }));
    expect(row?.id).toBeTruthy();
    const lidos = await withTenant(ctxCoordA, (tx) => tx.select().from(schema.bloqueio));
  ...
  test("coordenador cria bloqueio de paciente (viagem)", async () => {
    const [row] = await withTenant(ctxCoordA, (tx) =>
      tx.insert(schema.bloqueio).values({
        clinicId: CLINIC_A, escopo: "paciente", patientId: PAC_A1,
        dataInicio: "2026-07-20", dataFim: "2026-08-10", motivo: "viagem em família",
      }).returning({ id: schema.bloqueio.id }));
    expect(row?.id).toBeTruthy();
  });
  ...
  test("check barra escopo=clinica com terapeuta_id preenchido", async () => {
    await expect(withTenant(ctxCoordA, (tx) =>
      tx.insert(schema.bloqueio).values({
        clinicId: CLINIC_A, escopo: "clinica", terapeutaId: U_T1_A,
        dataInicio: "2026-07-20", dataFim: "2026-07-25", motivo: "x",
      }))).rejects.toThrow(); // constraint bloqueio_escopo_alvo
  });

  test("cross-tenant: coordenador B NÃO lê bloqueio da A", async () => {
    const lidos = await withTenant(ctxCoordB, (tx) => tx.select().from(schema.bloqueio));
  ...
  test("terapeuta NÃO cria bloqueio (só coordenação)", async () => {
    await expect(withTenant(ctxT1A, (tx) =>
      tx.insert(schema.bloqueio).values({
        clinicId: CLINIC_A, escopo: "clinica", dataInicio: "2026-09-07",
        dataFim: "2026-09-07", motivo: "feriado",
      }))).rejects.toThrow();
  });
});
