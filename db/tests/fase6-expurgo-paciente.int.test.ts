...
describe.skipIf(!hasDb)("fase6.3 · app_purgar_paciente + retenção", () => {
  ...
  test("app_role NÃO consegue UPDATE direto em audit_log (imutabilidade LGPD)", async () => {
    await expect(
      withTenant(ctx("coordenador", U_COORD_A), (db) =>
        db.execute(
          sql`UPDATE audit_log SET patient_id = NULL WHERE id = ${AUDIT_H1}::uuid`,
        ),
      ),
    ).rejects.toThrow();
  });
  ...
  test("paciente inexistente é rejeitado", async () => {
    await expect(
      withTenant(ctx("coordenador", U_COORD_A), (db) =>
        db.execute(
          sql`SELECT app_purgar_paciente('00000000-0000-0000-0000-0000000000ff'::uuid, 'x')`,
        ),
      ),
    ).rejects.toThrow();
  });
  ...
  test("elegibilidade: menor sob retenção = false; adulto vencido = true", async () => {
    const under = await withTenant(ctx("coordenador", U_COORD_A), (db) =>
      db.execute(
        sql`SELECT app_paciente_expurgavel(${P_UNDER}::uuid)::text AS elig`,
      ),
    );
    const past = await withTenant(ctx("coordenador", U_COORD_A), (db) =>
      db.execute(
        sql`SELECT app_paciente_expurgavel(${P_PAST}::uuid)::text AS elig`,
      ),
    );
  ...
  test("elegibilidade não vaza cross-tenant (coord A não enxerga paciente da clínica B)", async () => {
    ...
    const cross = await withTenant(ctx("coordenador", U_COORD_A), (db) =>
      db.execute(
        sql`SELECT app_paciente_expurgavel(${P2}::uuid)::text AS elig`,
      ),
    );
  ...
  test("coordenador purga: erasure físico + trilha pseudonimizada (não deletada)", async () => {
    await withTenant(ctx("coordenador", U_COORD_A), (db) =>
      db.execute(
        sql`SELECT app_purgar_paciente(${P1}::uuid, 'fim de retenção')`,
      ),
    );

    // erasure físico do paciente e descendentes bloqueantes
    expect(await owner!`SELECT 1 FROM patient WHERE id = ${P1}`).toHaveLength(
      0,
    );
    expect(
      await owner!`SELECT 1 FROM consent WHERE patient_id = ${P1}`,
    ).toHaveLength(0);
    expect(
      await owner!`SELECT 1 FROM report WHERE patient_id = ${P1}`,
    ).toHaveLength(0);
    expect(
      await owner!`SELECT 1 FROM report_pdf WHERE report_id = ${R1}`,
    ).toHaveLength(0);

    // trilha histórica: SOBREVIVE, mas pseudonimizada (patient_id NULL, sem PII)
    const hist =
      await owner!`SELECT patient_id, detalhe FROM audit_log WHERE id IN (${AUDIT_H1}, ${AUDIT_H2}) ORDER BY id`;
    ...
    expect(
      await owner!`SELECT 1 FROM audit_log WHERE patient_id = ${P1}`,
    ).toHaveLength(0);
