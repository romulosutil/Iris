...
> **Data:** 19/08/2026
> **Status:** 🟢 **#388, #387, #390 e #389 mergeadas na main (PRs #397, #398, #399, #400). #391 rebaseada limpa na main, verificada e pronta (PR #401).** #391: coluna `origem` em `alerta_risco_clinico` (migração `0111`), CHECK relaxado por origem sem afrouxar FK composta anti-IDOR, caminho não-LLM p/ item de risco de instrumento formal (≥1/`null` dispara, `0` não), gatilho no RPD transversal, `SYSTEM_PROMPT` padrão ganhou regra de risco. Verificação em 2 rodadas de fork achou 2 gaps de DoD (imutabilidade de alerta em edição de RPD — feature de edição **não existe**, invariante vale por ausência; RLS sem cobrir origens novas — corrigido, 2 testes novos). Bug real achado e corrigido: ordem de cleanup em `actions.int.test.ts` violava FK nova. Verde: `pnpm typecheck/lint/test`. Detalhe completo no `BACKLOG.md`. Próximo: #392.
>
