...
| T30  | `feat(timeline)` | `sessionNumero > 1` alterado para `sessionNumero > 0` em `carregarDeltaSessao` (`queries.ts:313`), permitindo que a Sessão 1 compare contra o marco 0. Comentário explicativo adicionado sobre `n = 0` não ter anterior. `pnpm typecheck` limpo. |
...
**Progresso: 12 de 34 tasks completas e revisadas** (T01-T10, T30, T32). Fase 0 fechada. Fase 1 e Fase 3 em andamento.
