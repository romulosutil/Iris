...
### A Solução Iris: *"Chegue na avaliação com o dossiê pronto"*
...
* **Separabilidade Estrita:** Há uma separação rígida entre a *evidência* (um fato observado na sessão) e a *pontuação formal* (marcos consolidados em janelas de avaliação pelo coordenador).
...
* **Papel Ativo Determinístico:** O acesso de um usuário a determinadas telas e ações é computado em tempo de execução no servidor (`papelAtivo`). Um cookie apenas *indica* qual clínica/papel o usuário escolheu, mas quem de fato *autoriza* a ação é a verificação estrita do banco contra a tabela `user_role` a cada requisição.
