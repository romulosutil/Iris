...
Desatrelar o expurgo de `audit_log` da exclusão da conta do usuário ou tenant. Respaldado pelo Art. 7º, II da LGPD (_cumprimento de obrigação legal_), os eventos de autenticação e acesso na tabela `audit_log` permanecerão retidos por 6 meses antes da exclusão física definitiva.
