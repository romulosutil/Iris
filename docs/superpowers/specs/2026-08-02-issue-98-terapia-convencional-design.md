...
- ✅ **Formulário de Consentimento LGPD (Titular Adulto)**: Concluída a implementação do componente `RadioCards` em `novo-paciente-form.tsx`, garantindo seleção explícita entre *O próprio paciente (titular adulto)* e *Responsável legal*.
- 🎨 **Redesenho do Seletor de Abordagem**: Definido o modelo de *Seletor de Modo de Atendimento* (Terapia Convencional vs Protocolos de Marcos) para a ficha clínica do paciente, eliminando o acúmulo de cards inativos.
...
* **Seleção Opcional:** Na tela de cadastro de metas/plano do paciente, adicionar a opção bem destacada: *"Psicologia Generalista / Atendimento Sem Protocolo Rígido"*.
* **Visualização da Sessão:** A aba de evidências quantitativas é substituída pela aba *"Linha do Tempo & Resumos da Sessão"*.
