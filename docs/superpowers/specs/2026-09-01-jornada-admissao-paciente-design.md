...
> Cada edição está marcada `(auditoria 02/09, R-n)`.
>
> **Ratificação 03/09/2026** — **D-A6, D-A8, D-A9 e D-A10 estão ratificados
> pelo Rômulo.** Os quatro entraram depois da ratificação de 01/09 e já estão
> em produção desde a PR #557 (commit `a3142384`). **D-A10 foi resolvido pela
> opção (b)**: o definer `app_fatos_prontidao` (migração
> `db/migrations/0149_fatos_prontidao_definer.sql`) enxerga o terapeuta **da
> sessão**, com guard interno espelhando o predicado da policy de leitura.
>
> **Aberto após a #557:** os buracos de prova e os desvios de implementação
> estão catalogados em
> [`sequela-557-provas-e-desvios.md`](./sequela-557-provas-e-desvios.md).
> Entre eles, o contrato de `obterFatosProntidao` — a spec (§4a) manda
> devolver `FatosProntidao | null`; a implementação de hoje **lança**. Quem
> muda é a implementação, não a spec.
...
**Quem desbloqueia `cognitive_behavioral`** _(auditoria 02/09, R-8 —
ratificado pelo Rômulo em 03/09/2026)_ — o degrau bloqueante é "instrumento aplicado", que é
...
| `pendente`     | neutro: **sem `bg` próprio** (fundo do cartão) / `--text-secondary` / `--border-brutal` | "Recomendado"  | Ausência de dado não-causal; não pede atenção. **Correção de 03/09/2026 (PR #572):** a redação original fixava `--surface-muted` como fundo, mas o par `--text-secondary` (#71717a) sobre `--surface-muted` (#f1efe9) mede **4.20:1** no tema claro — abaixo do piso AA de 4.5:1, porque o selo é 12px semibold e não conta como texto grande. O mesmo par sobre o fundo do cartão mede 4.83:1 e passa; o dark já passava (5.81:1). `axe` sob jsdom **não** checa contraste, então a reprovação não apareceria no CI. Trocado só o portador do neutro; a intenção (não disputar atenção com o degrau bloqueante) está preservada. **Não reverter para `--surface-muted`** sem antes escurecer `--text-secondary`, o que respinga no app inteiro e é escopo do DS (#566). |
