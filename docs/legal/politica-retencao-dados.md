...
| **Cópias de segurança** (MinIO local + OCI S3 off-site — #89)                  | 30 dias                                                                     | LGPD Art. 46 (segurança e recuperação)                                          | Local/MinIO: prune do `infra/backup/backup.sh` (`RETENTION_DAYS`, default 30). Off-site: expurgo e auditoria automatizados via `infra/backup/expurgo-offsite.sh` (regra de lifecycle OCI S3 + script de auditoria/expurgo) | Implementado (`expurgo-offsite.sh` / `test-offsite.sh`)              |
...
| Lifecycle Rule de 30 dias no bucket off-site (OCI S3) | Implementado e auditado via `infra/backup/expurgo-offsite.sh` e `infra/backup/test-offsite.sh`. O script audita a retenção de 30d no bucket off-site e executa o expurgo ativo de artefatos expirados. | Fechado (#354)        |
