...
> ⚠️ **PIVÔ DE HOSPEDAGEM EM AVALIAÇÃO (09/07/2026) — NÃO TRAVADO.** A camada
> de **hospedagem/deploy** deste documento (Vercel `gru1` + Supabase
> gerenciado `sa-east-1`, "sem containers") está sob revisão: nova premissa é
> **VPS Hostinger + Easypanel (Docker), Postgres puro** (não Supabase). Enquanto não
> confirmado, trate a tabela §1 (linhas de Host/Deploy) e o "não usar
> containers" (§2) como **candidatos a substituição** — a proposta detalhada e
> as decisões de tech lead estão em
> [`plano-bootstrap-e-stack-vps.md`](plano-bootstrap-e-stack-vps.md). **O resto
> deste documento (framework, modelo de dados, RLS, plano de fases 0.5→6,
> definição de pronto) permanece válido e intacto.**
